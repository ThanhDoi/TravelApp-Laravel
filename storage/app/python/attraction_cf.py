...
r = requests.post('http://travelapp.test/api/getAttractionCFData')
...
attraction_ids = r.json()['attractions']
attraction_ids = attraction_ids.split('|')
attraction_ids = [int(i) for i in attraction_ids]
attraction_ids = np.array(attraction_ids).reshape(-1, 1)
...
Y_data = np.c_[user_ids, attraction_ids, ratings]
